...
float getCorrectReflectance(float rTOA, int method, float threshold);
...
int main(int argc, char* argv[])
{
	...
	if(argc!=4) 
	{
		...
		fputs("\nMethod: 1 -- simple clipping if outside interval 0-1 [if(Value<0) OutValue=0; if(Value>1) OutValue=1;]\n", stderr);
						...
						if( (hBandIn = GDALGetRasterBand(hDatasetIn, band)) != NULL )
						{
							if( (hBandOut = GDALGetRasterBand(hDatasetOut, band)) != NULL )
							{
								...
								for(int i=0; i<rows; i++)
								{
									...
									for(int j=0; j<cols; j++) pLineOut[j] = getCorrectReflectance(pLineIn[j], method, vThreshold);
